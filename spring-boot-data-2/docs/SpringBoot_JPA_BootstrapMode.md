...
对于下面的例子，**我们将使用@DataJpaTest注解并说明不同的Repository启动选项**。
...
让我们使用ThreadPoolTaskExecutor(它的Spring实现之一)在配置类中声明一个AsyncTaskExecutor，并重写返回Future的submit方法：
...
在本例中，应用程序上下文启动完成触发Repository的初始化。
